...
class ConvDRAW(object):
    ...
    def __init__(self, q, q_rnn, p, p_rnn, write, n_batch, optimizer,
                 glimpses=64, random=1234):
        self.q = q
        self.q_rnn = q_rnn
        self.p = p
        self.p_rnn = p_rnn
    ...
    def step(self, cell_enc, cell_dec, hid_enc, hid_dec,
        ...
        x_err = x - self.p.fprop(
            [canvas], self.srng, deterministic=deterministic)

        new_cell_enc, new_hid_enc = self.q_rnn.fprop(
            [x, x_err, cell_enc, hid_enc, hid_dec], self.srng,
            deterministic=deterministic)
        mean, var = self.q.fprop(
            [new_hid_enc], self.srng, deterministic=deterministic)
        kl = gauss_unitgauss_kl(mean, var).mean()

        z = self.q.sample_given_x(
            [new_hid_enc], self.srng, deterministic=deterministic)[-1]

        # decoder
        new_cell_dec, new_hid_dec = self.p_rnn.fprop(
            [z, cell_dec, hid_dec],
            self.srng,
            deterministic=deterministic)

        # write
        new_canvas = self.write.fprop(
            [new_hid_dec], self.srng, deterministic=deterministic)
        new_canvas = canvas + new_canvas

        return (new_cell_enc, new_cell_dec,
                new_hid_enc, new_hid_dec, new_canvas, z, kl)
    ...
    def lowerbound(self):
        ...
        outputs_info = [init_cell_enc, init_cell_dec,
                        init_hid_enc, init_hid_dec, init_canvas, None, None]
        ...
        params = p_rnn_params + p_params \
            + q_rnn_params + q_params + write_params
        ...
        canvas_all = self.p.fprop(
            [canvas.dimshuffle(1, 0, 2, 3, 4)], self.srng, deterministic=True)
    ...
    def p_sample_mean_given_x(self):
        ...
        init_canvas = T.ones(
            (z.shape[0],) + self.write.mean_network.output_shape[1:])

        def p_step(z, cell_dec, hid_dec, canvas):
            # decoder
            new_cell_dec, new_hid_dec = self.p_rnn.fprop(
                [z, cell_dec, hid_dec], self.srng, deterministic=True)

            # write
            # TODO : Fix error
            new_canvas = self.write.fprop(
                [new_hid_dec], self.srng, deterministic=True
