...
def mnist(datapath,toFloat=False):
    ...
    def load(test=False,one_hot=True):
        ...
        if toFloat:
            ...
            p.std[p.std==0]=1
            train_x = ((train_x-p.mean[np.newaxis])/p.std[np.newaxis]).astype(np.float32)
            test_x = ((test_x-p.mean[np.newaxis])/p.std[np.newaxis]).astype(np.float32)
...
def svhn(datapath,toFloat=True,binarize_y=True,gray=False,extra=True):
    ...
    def load(test=False,flatten=False):
        ...
        if gray is True:
            train_x = train_x[:,0]*0.2126 + train_x[:,1]*0.7152 + train_x[:,2]*0.0722
            test_x = test_x[:,0]*0.2126 + test_x[:,1]*0.7152 + test_x[:,2]*0.0722

            train_x = (train_x/255.).reshape((len(train_x),32*32)).astype(np.float32)
            test_x = (test_x/255.).reshape((len(test_x),32*32)).astype(np.float32)
...
def lfw(datapath,toFloat=True,gray=False,rate=0.1,rseed=0):
    ...
    def load(test=False):
        ...
        if test:
            train_x, test_x, train_y, test_y = train_test_split(x, y, test_size=rate, random_state=rseed)
        else:
            train_x = x
            train_y = y
