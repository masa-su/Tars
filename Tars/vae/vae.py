...
class VAE(object):
    ...
    def lowerbound(self,random):
        ...
        loglike, p_param = self.p.log_likelihood_given_x(rep_x[0],[h]+rep_x[1:]) #p(x|z,y,...)
        ...
        self.lowerbound_train = theano.function(inputs=x, outputs=lowerbound, updates=updates, on_unused_input='ignore')
    ...
    def lowerbound_renyi(self,alpha,random):
        ...
        x = self.q.inputs
        rep_x = [t_repeat(_x, self.l, axis=0) for _x in x]
        q_samples = self.q.sample_given_x(rep_x,self.srng,deterministic=False)
        log_iw,params = self.log_importance_weight(rep_x,q_samples)

        log_iw_matrix = log_iw.reshape((x[0].shape[0], self.k))
        log_iw_minus_max = log_iw_matrix - T.max(log_iw_matrix, axis=1, keepdims=True)
        iw = T.exp(log_iw_minus_max)
        iw = iw**(1-alpha)
        iw_normalized = iw / T.sum(iw, axis=1, keepdims=True) #(x[0].shape[0],k)

        lowerbound = T.mean(log_iw)

        if alpha == -np.inf:
            gparams = [T.grad(-T.sum(T.max(log_iw_matrix,axis=1)), param) for param in params]

        else:
            iw_normalized_vector = T.reshape(iw_normalized, log_iw.shape) #(x[0].shape[0]*num_samples)
    ...
    def log_likelihood_test(self,test_set,l=1,k=1,mode='iw'):
        ...
        get_log_likelihood = theano.function(inputs=x, outputs=log_likelihood, on_unused_input='ignore')
    ...
    def p_sample_mean_given_x(self):
        ...
        self.p_sample_mean_x = theano.function(inputs=x, outputs=samples, on_unused_input='ignore')

    def q_sample_mean_given_x(self):
        x = self.q.inputs
        samples = self.q.sample_mean_given_x(x,deterministic=True)
        self.q_sample_mean_x = theano.function(inputs=x, outputs=samples, on_unused_input='ignore')
    ...
    def log_marginal_likelihood_iwae(self,x,k):
        ...
        log_marginal_estimate = LogMeanExp(log_iw_matrix, axis=1, keepdims=True)

        return log_marginal_estimate
    
    def log_importance_weight(self,rep_x,samples):
        log_iw = 0
        p_log_likelihood,p_param = self.p.log_likelihood_given_x(rep_x[0],[samples]+rep_x[1:]) #p(x|z,y,...)
        q_log_likelihood,q_param = self.q.log_likelihood_given_x(samples,rep_x) #p(z|x,y,...)
